...
const categoryCards = [["영어회화", "녹화 강의가 아닌 실시간 수업으로 선생님과 충분히 듣고 말합니다.", "english-conversation", englishImage], ["일본어회화", "이동 없이 집에서 기초 표현부터 상황별 일본어 대화까지 연결합니다.", "japanese-conversation", japaneseImage], ["시험 대비", "현재 실력과 목표 시험일에 맞춰 필요한 영역과 오답을 1:1로 관리합니다.", "exam", examImage], ["비즈니스", "실시간 화상수업에서 회의와 이메일 등 실제 업무 장면을 연습합니다.", "business-english", businessImage]].map(([title, description, lessonType, image]) => `<article class="category-card">${renderImageBox(image, "image-box-square")}<div class="category-card-body"><p class="card-meta">온라인 1:1 대표 수업</p><h3>${title}</h3><p>${description}</p><button class="text-link category-region-trigger" type="button" data-lesson-region-trigger="${lessonType}" aria-haspopup="dialog">내 지역 수업 보기 →</button></div></article>`).join("");
