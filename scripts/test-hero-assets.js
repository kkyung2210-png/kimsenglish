...
function runTest() {
  ...
  const desktopExists = fs.existsSync(
    path.join(ROOT_DIR, 'public', 'images', 'hero.webp')
  );
  const mobileExists = fs.existsSync(
    path.join(ROOT_DIR, 'public', 'images', 'hero-mobile.webp')
  );
  ...
  try {
    ...
    fs.writeFileSync(path.join(fixtureHeroDir, 'hero.webp'), tinyWebp);
    ...
    assert.match(webpHtml, /src="\/images\/hero\.webp"/);
    assert.match(webpHtml, /srcset="\/images\/hero-mobile\.webp"/);
  ...
  console.log('- Mobile 이미지:', mobileExists ? '전용 WebP 사용' : 'Desktop WebP 자동 대체');
