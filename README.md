...
검색 결과 설명은 `slug`를 기준으로 고정된 패턴을 선택합니다. 같은 페이지를 다시 만들어도 같은 문장이 나오며, 회화·시험·비즈니스·여행마다 6개 이상의 문장 틀을 사용합니다.

각 지역 페이지는 키워드를 억지로 반복하지 않습니다. 대신 지역과 과목 및 대상을 명확하게 설명하고 핵심 요약과 FAQ 및 관련 수업 링크를 제공해 사람과 검색엔진이 내용을 쉽게 이해하도록 구성합니다.
...
지역 페이지에는 AI Summary, 추천 대상, Key Takeaways와 다음 추천 페이지가 일반 본문으로 함께 생성됩니다. 이 내용은 사람과 검색 서비스가 페이지의 대상, 수업 내용과 기대 변화를 빠르게 이해할 수 있도록 `pages.csv`의 콘텐츠 값을 자연스러운 문장으로 정리합니다.
